...
function showMapFallback(el, lat, lng, hasCoords) {
  el.innerHTML = '<div class="flex flex-col items-center justify-center h-full bg-gray-100 rounded-xl p-6 text-center">' +
    ...
    (hasCoords ? '<a href="https://serpapi.com/search?engine=google_maps&?q=' + lat + ',' + lng + '" target="_blank" class="mt-3 text-primary hover:text-primary-dark text-sm font-semibold">عرض على Google Maps</a>' : '') +
    '</div>';
}
